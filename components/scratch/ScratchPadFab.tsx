...
// ScratchPadFab — draggable circular floating action button for the Quick Notes pad.
// Construction mirrors ChatFab (tap to open, drag to reposition, position persisted
// in localStorage). Default position sits above the ChatFab so the two don't overlap.

import { useState, useRef, useEffect, useCallback } from 'react'
import { NotebookPen } from 'lucide-react'
import { ScratchPadSheet } from '@/components/scratch/ScratchPadSheet'

const LS_KEY = 'sentinel-scratch-fab-pos'
...
function defaultPosition() {
  // Same column as ChatFab, but one FAB + gap higher so they stack without overlap.
  return {
    x: window.innerWidth - FAB_SIZE - PADDING - 80,
    y: window.innerHeight - FAB_SIZE - PADDING - 160 - (FAB_SIZE + 72),
  }
}
...
export function ScratchPadFab() {
  ...
  return (
    <>
      ...
      <ScratchPadSheet open={open} onOpenChange={setOpen} />
    </>
  )
}
